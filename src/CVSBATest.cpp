...
Modified by Pablo Ramon Soria (prs@us.es).
...
void visualize(std::vector<cv::Point3f>_points, std::vector<cv::Mat> _ts, std::vector<cv::Mat>  _rs);
...
void visualize(std::vector<cv::Point3f> _points, std::vector<cv::Mat> _ts, std::vector<cv::Mat>  _rs){
    ...
    for(unsigned i = 0; i < _rs.size(); i++){
        Eigen::Affine3f pose;
        cv::Mat rod;
        cv::Rodrigues(_rs[i], rod);

        Eigen::Matrix4f rotation;
        rotation.setZero();

        rotation(0,0) = rod.at<double>(0,0);
        rotation(0,1) = rod.at<double>(0,1);
        rotation(0,2) = rod.at<double>(0,2);
        rotation(1,0) = rod.at<double>(1,0);
        rotation(1,1) = rod.at<double>(1,1);
        rotation(1,2) = rod.at<double>(1,2);
        rotation(2,0) = rod.at<double>(2,0);
        rotation(2,1) = rod.at<double>(2,1);
        rotation(2,2) = rod.at<double>(2,2);
        rotation(0,3) = _ts[i].at<double>(0);
        rotation(1,3) = _ts[i].at<double>(1);
        rotation(2,3) = _ts[i].at<double>(2);
        rotation(3,3) = 1;

        pose.matrix() = rotation;

        viewer.addCoordinateSystem(0.001, pose, "camera_" + std::to_string(i));
    }

    viewer.addCoordinateSystem(0.0005, "origin");
